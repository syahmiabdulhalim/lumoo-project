...
class ProductServiceTest {
    ...
    void getAllApprovedList_delegatesToRepo() {
        when(productRepository.findByApprovedTrueWithVendor()).thenReturn(List.of(product));
        List<Product> result = productService.getAllApprovedList();
        assertEquals(1, result.size());
        verify(productRepository).findByApprovedTrueWithVendor();
    }
    @Test
    void searchApproved_delegatesToRepo() {
        when(productRepository.findByApprovedTrueAndNameContainingIgnoreCaseWithVendor("hat"))
    ...
    void getApprovedByCategory_delegatesToRepo() {
        when(productRepository.findByCategoryIgnoreCaseAndApprovedWithVendor("CLOTHES"))
                .thenReturn(List.of(product));
